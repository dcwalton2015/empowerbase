...
if ( ! function_exists( 'clean_box_custom_header' ) ) :
/**
 ...
 */
	function clean_box_custom_header() {
		/**
		...
		if ( 'light' == $options['color_scheme'] ) {
			$default_header_color = clean_box_get_default_theme_options();
			$default_header_color = $default_header_color['header_textcolor'];
		}
		else if ( 'dark' == $options['color_scheme'] ) {
			$default_header_color = clean_box_default_dark_color_options();
			$default_header_color = $default_header_color['header_textcolor'];
		}
		...
		// Set height and width, with a maximum value for the width: 21:9
		'height'                 => 514,
		'width'                  => 1200,
		...
		'wp-head-callback'       => 'clean_box_header_style',
		'admin-head-callback'    => 'clean_box_admin_header_style',
		'admin-preview-callback' => 'clean_box_admin_header_image',
	);
...
endif; // clean_box_custom_header
add_action( 'after_setup_theme', 'clean_box_custom_header' );


if ( ! function_exists( 'clean_box_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog
 *
 * @see clean_box_custom_header()
 *
...
endif; // clean_box_header_style


if ( ! function_exists( 'clean_box_admin_header_style' ) ) :
/**
 * Styles the header image displayed on the Appearance > Header admin panel.
 *
 * @since Clean Box 0.1
 */
function clean_box_admin_header_style() {
	$options 	= clean_box_get_theme_options();
...
endif; // clean_box_admin_header_style


if ( ! function_exists( 'clean_box_admin_header_image' ) ) :
/**
 * Styles the header image displayed on the Appearance > Header admin panel.
 *
 * @since Clean Box 0.1
 */
function clean_box_admin_header_image() {
...
endif; // clean_box_admin_header_image

if ( ! function_exists( 'clean_box_site_branding' ) ) :
	/**
	 * Get the logo and display
	 *
	 * @uses get_transient, clean_box_get_theme_options, get_header_textcolor, get_bloginfo, set_transient, display_header_text
...
endif; // clean_box_site_branding
add_action( 'clean_box_header', 'clean_box_site_branding', 70 );


if ( ! function_exists( 'clean_box_featured_image' ) ) :
	/**
	 * Template for Featured Header Image from theme options
	 *
	 * To override this in a child theme
	 * simply create your own clean_box_featured_image(), and that function will be used instead.
	 *
	 * @since Clean Box 0.1
	 */
	function clean_box_featured_image() {
		$options				= clean_box_get_theme_options();
		...
		if ( !$clean_box_featured_image = get_transient( 'clean_box_featured_image' ) ) {
			...
			if ( '' != $header_image  ) {
				...
				$clean_box_featured_image .= '</div><!-- .wrapper -->
				</div><!-- #header-featured-image -->';
			}

			set_transient( 'clean_box_featured_image', $clean_box_featured_image, 86940 );
		}

		echo $clean_box_featured_image;

	} // clean_box_featured_image
endif;


if ( ! function_exists( 'clean_box_featured_page_post_image' ) ) :
	/**
	 * Template for Featured Header Image from Post and Page
	 *
	 * To override this in a child theme
	 * simply create your own clean_box_featured_imaage_pagepost(), and that function will be used instead.
	 *
	 * @since Clean Box 0.1
	 */
	function clean_box_featured_page_post_image() {
		...
		if( has_post_thumbnail( $header_page_id ) ) {
			...
			if ( 'featured-header' ==  $featured_image_size ) {
				$feat_image = get_the_post_thumbnail( $post->ID, 'clean-box-featured-header', array('id' => 'main-feat-img'));
			}
			else if ( 'featured' ==  $featured_image_size ) {
				$feat_image = get_the_post_thumbnail( $post->ID, 'clean-box-featured', array('id' => 'main-feat-img'));
			}
			else {
				$feat_image = get_the_post_thumbnail( $post->ID, 'clean-box-featured-grid', array('id' => 'main-feat-img'));
			}
			...
			$clean_box_featured_image .= '</div><!-- #header-featured-image -->';
...
if ( ! function_exists( 'clean_box_featured_overall_image' ) ) :
	/**
	 * Template for Featured Header Image from theme options
	 *
	 * To override this in a child theme
	 * simply create your own clean_box_featured_pagepost_image(), and that function will be used instead.
	 *
	 * @since Clean Box 0.1
	 */
	function clean_box_featured_overall_image() {
		global $post, $wp_query;
		$options				= clean_box_get_theme_options();
		$defaults 				= clean_box_get_default_theme_options();
	...
	} // clean_box_featured_overall_image
endif;
add_action( 'clean_box_after_header', 'clean_box_featured_overall_image', 10 );