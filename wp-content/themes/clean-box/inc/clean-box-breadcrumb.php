 ...
 * Add breadcrumb.
 *
 * @action clean_box_after_header
 *
 * @since Clean Box 0.1 
 */
if( !function_exists( 'clean_box_add_breadcrumb' ) ) :

	function clean_box_add_breadcrumb() {
		...
		if( isset ( $options['breadcumb_option'] ) && $options['breadcumb_option'] ){
			...
			echo clean_box_custom_breadcrumbs( $showOnHome, $delimiter );
		}
		else
			return false;
	}
...
add_action( 'clean_box_after_header', 'clean_box_add_breadcrumb', 50 );
 ...
 * @since Clean Box 0.1 
 */
if( !function_exists( 'clean_box_custom_breadcrumbs' ) ) :

	function clean_box_custom_breadcrumbs( $showOnHome, $delimiter ) {

		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'clean-box' ); // text for the 'Home' link
		$text['category'] = __( '%1$s Archive for %2$s', 'clean-box' ); // text for a category page
		$text['search']   = __( '%1$sSearch results for: %2$s', 'clean-box' ); // text for a search results page
		$text['tag']      = __( '%1$sPosts tagged %2$s', 'clean-box' ); // text for a tag page
		$text['author']   = __( '%1$sView all posts by %2$s', 'clean-box' ); // text for an author page
		$text['404']      = __( 'Error 404', 'clean-box' ); // text for the 404 page
		...
		if( is_front_page() ) {
			...
			if( get_query_var( 'paged' ) ) {
				...
				echo sprintf( __( 'Page %s', 'clean-box' ), max( $paged, $page ) );
				if( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
	...
	} // end clean_box_breadcrumb_lists
endif;